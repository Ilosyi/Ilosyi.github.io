...
如果不想直接在代码库里编辑 Markdown 和配置文件，可以启动本地 admin 界面：

```bash
pnpm admin
```

启动后访问 `http://127.0.0.1:4322`。该界面只绑定本机地址，用于本地管理博客内容，不会随站点构建发布。

admin 界面当前支持：

- 文章管理：搜索文章、新建草稿、编辑 frontmatter、编辑正文 Markdown、查看预览、保存文章。
- 分类管理：分类输入支持 `/` 表示嵌套分类，例如 `笔记/后端`；保存时会根据 `config/site.yaml` 的分类映射移动文章文件位置。
- 封面设置：文章封面可选择“自动随机”，也可从 `/img/cover/` 封面池中选定一张。
- 配置管理：以表单或原始 JSON 方式编辑 `config/site.yaml`。
- 图片管理：上传站点头图、周刊头图、文章封面；新增文章封面会按现有数字顺序保存为下一个 `.webp` 文件名。
...
- **本地 Admin 管理界面**：支持文章、配置和图片资源管理，避免日常写作时直接修改代码文件
...
- **[Admin 管理界面技术说明](./docs/overview/11-admin-workbench.md)** - 本地管理界面的架构、文件写入规则和维护说明
